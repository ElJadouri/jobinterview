...
OrderOptimizerNode::OrderOptimizerNode() : Node("OrderOptimizer")
{
  ...
  currentPositionSub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
      "currentPosition", 10, std::bind(&OrderOptimizerNode::msgCurrentPos, this, std::placeholders::_1));

  nextOrderSub_ = this->create_subscription<msg_package::msg::Order>(
...
//------------------------------------------------------------------------------------------------------------------------------------
std::vector<std::pair<float, Part>> OrderOptimizerNode::FindShortestPath(OrderDetails details)
{
